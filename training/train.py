...
def main():
    ...
    # unified prompting for NeoBabel
    ...
    # Initialize NeoBabel model
    if config.model.neobabel.get("load_from_neobabel", False):
        model = NeoBabel.from_pretrained(config.model.neobabel.pretrained_model_path).to(accelerator.device)
        if config.model.neobabel.vocab_size != model.vocab_size:
            model.neobabel.resize_token_embeddings(config.model.neobabel.vocab_size)
            model.config.codebook_size = config.model.neobabel.codebook_size
            model.config.vocab_size = config.model.neobabel.vocab_size
            model.vocab_size = config.model.neobabel.vocab_size
            model.output_size = config.model.neobabel.vocab_size
            model.config.mask_token_id = config.model.neobabel.vocab_size - 1
            model.mask_token_id = config.model.neobabel.vocab_size - 1
    else:
        model = NeoBabel(**config.model.neobabel).to(accelerator.device)
    ...
    if config.experiment.resume_from_checkpoint:
        ...
        if path is not None:
            checkpoint_path = os.path.join(config.experiment.output_dir, path)
            
            # Check if it's checkpoint-0 or another checkpoint
            if path == "checkpoint-0":
                # Only load model weights for checkpoint-0
                accelerator.print(f"Found checkpoint-0. Loading only model weights from {checkpoint_path}")
                
                # Check for different possible model file locations
                model_path = None
                if os.path.exists(f'{checkpoint_path}/unwrapped_model/pytorch_model.bin'):
                    model_path = f'{checkpoint_path}/unwrapped_model/pytorch_model.bin'
                else:
                ...
                if model_path:
                    ...
                    # Check if the state dict has 'module.' prefix and remove it if necessary
                    if any(key.startswith('module.') for key in state_dict.keys()):
                        accelerator.print("Removing 'module.' prefix from state dict keys")
                        state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
                    
                    # Load model weights before preparing with accelerator
                    model.load_state_dict(state_dict, strict=True)
                    del state_dict
                else:
                    logger.warning(f"No model weights found in {checkpoint_path}")
                
                # Start training from scratch with prepared models
                model, optimizer, lr_scheduler = accelerator.prepare(model, optimizer, lr_scheduler)
                global_step = 0
                first_epoch = 0
            else:
                # For other checkpoints, load complete training state
                accelerator.print(f"Resuming from checkpoint {checkpoint_path}")
                
                # Load metadata first to get global_step
...
def visualize_predictions(
    ...
    predictions = logits[:config.training.batch_size_t2i, -(config.model.neobabel.num_vq_tokens + 1):-1:,
                  config.model.neobabel.llm_vocab_size + config.model.neobabel.num_new_special_tokens:-1]
    predictions = predictions.argmax(axis=-1)

    mask_token_id = config.model.neobabel.vocab_size - 1 - len(uni_prompting.text_tokenizer)
    input_ids = input_ids[:config.training.batch_size_t2i, -(config.model.neobabel.num_vq_tokens + 1):-1:] - len(
        uni_prompting.text_tokenizer)
    mask_ratio = list((torch.where(input_ids == mask_token_id, 1, 0).sum(
        dim=-1) / config.model.neobabel.num_vq_tokens).cpu().numpy())
...
def generate_images(
    ...
    mask_token_id = config.model.neobabel.vocab_size - 1
    batch_size = config.training.get("validation_batch_size", 8)  # Define a batch size
    ...
    for i in range(0, len(validation_prompts), batch_size):
        ...
        image_tokens = torch.ones((len(batch_prompts), config.model.neobabel.num_vq_tokens), dtype=torch.long,
        ...
        wandb_images.extend([wandb.Image(image, caption=batch_prompts[j]) for j, image in enumerate(batch_pil_images)])
...
def save_checkpoint(model, config, accelerator, global_step):
    ...
    # Use accelerator.save_state() to save complete training state
    accelerator.save_state(save_path)
    
    # Save metadata
    if accelerator.is_main_process:
        json.dump({"global_step": global_step}, (save_path / "metadata.json").open("w+"))
        logger.info(f"Saved complete state to {save_path}")
