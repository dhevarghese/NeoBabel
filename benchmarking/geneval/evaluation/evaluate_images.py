...
def parse_args():
    ...
    parser.add_argument("--wandb-project", type=str, default="inference-scaling-mcts", help="WandB project name")
    parser.add_argument("--wandb-entity", type=str, default="d-varghese", help="WandB entity name")
    parser.add_argument("--object-names", type=str, default="object_names.txt", help="Path to object names file")
    # Other arguments
...
def load_models(args):
    ...
    with open(os.path.join(os.path.dirname(__file__), args.object_names)) as cls_file:
...
def main(args):
    wandb.init(project=args.wandb_project, entity=args.wandb_entity, name=args.outfile.split(".")[0].split("/")[-1])
    incorrect_images_table = wandb.Table(columns=["Image", "Image ID", "Tag", "Prompt", "Reason"])

    full_results = []
    for subfolder in tqdm(os.listdir(args.imagedir), desc="Processing images"):
        ...
        image_list = [f for f in os.listdir(os.path.join(folderpath, "samples")) 
              if os.path.isfile(os.path.join(folderpath, "samples", f)) and re.match(r"\d+\.png", f)]
        ...
        for imagename in image_list:
            ...
            # Upload incorrect images to WandB
            if not result["correct"]:  # If incorrectly classified
                incorrect_images_table.add_data(
                    wandb.Image(imagepath, caption=f"Tag: {result['tag']}\nPrompt: {result['prompt']}"),
                    result["filename"].split('/')[-3],  # Image ID
                    result["tag"],  # Image tag
                    result["prompt"],  # Text prompt
                    result["reason"]  # Failure reason
                )
            
    wandb.log({"Incorrect Images": incorrect_images_table})
    ...
    with open(args.outfile, "w", encoding="utf-8") as fp:
        pd.DataFrame(full_results).to_json(fp, orient="records", lines=True, force_ascii=False)
